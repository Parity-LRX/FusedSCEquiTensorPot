...
from molecular_force_field.models import E3_TransformerLayer_multi, CartesianTransformerLayer, CartesianTransformerLayerStrict, CartesianTransformerLayerLoose, MainNet
...
def main():
    ...
    parser.add_argument('--tensor-product-mode', type=str, default='spherical',
                        ...
                        help='Tensor product mode: "spherical" uses e3nn spherical harmonics (default), '
                             '"cartesian" uses Cartesian tensor products with EquivariantTensorProduct (strictly equivariant), '
                             '"cartesian-strict" uses strict parity Cartesian products (physically rigorous), '
                             '"cartesian-loose" uses optimized CartesianFullyConnectedTP (fastest, not strictly equivariant)')
    ...
    if args.tensor_product_mode == 'cartesian':
        logging.info("Using Cartesian tensor product mode (faster, fewer parameters)")
        ...
        logging.info("Using Cartesian tensor product mode with STRICT parity (physically rigorous)")
