...
def init_weighted_sum_weights(model):
    ...
    for name, module in model.named_modules():
        if hasattr(module, 'weights') and hasattr(module, 'num_features'):
            # 这是 RobustScalarWeightedSum
            if module.weights is not None:
                with torch.no_grad():
                    # 随机初始化权重，确保输出非零
                    module.weights.data = torch.randn(module.num_features, device=module.weights.device, dtype=module.weights.dtype) * 0.1
...
def test_equivariance(
    ...
    with torch.no_grad():
        ...
        # 检查输出是否非零
        if torch.abs(E_orig).max().item() < 1e-10:
            print(f"    WARNING: Model output is near zero, re-initializing weighted_sum weights...")
            init_weighted_sum_weights(model)
            E_orig = model(pos, A, batch, edge_src, edge_dst, edge_shifts, cell)
            if torch.abs(E_orig).max().item() < 1e-10:
                print(f"    ERROR: Model output is still near zero after re-initialization!")
                return float('inf')
        ...
        try:
            ...
            with torch.no_grad():
                ...
                if torch.abs(test_output).max().item() < 1e-10:
                    print(f"  WARNING: Model output is near zero, re-initializing...")
                    init_weighted_sum_weights(model)
                    test_output = model(pos, A, batch, edge_src, edge_dst, edge_shifts, cell)
                    if torch.abs(test_output).max().item() < 1e-10:
                        print(f"  ERROR: Model output is still near zero!")
            ...
            max_equiv_err = test_equivariance(
                model, pos, A, batch, edge_src, edge_dst, edge_shifts, cell, n_tests=20
            )
            print(f"  Max equivariance error (O(3), including parity): {max_equiv_err:.2e}")
